...
class CTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if file_path.endswith('.nii.gz'):
            ...
            spacing = header.get('spacings', None) or [np.linalg.norm(direction) for direction in header.get('space directions', [])]
            if spacing:
...
def get_data_loaders(noncontrast_dir, contrast_dir, test_noncontrast_dir, batch_size, patch_size=128):
    ...
    noncontrast_loader = DataLoader(noncontrast_dataset, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=False)
